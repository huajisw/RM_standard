...
static void Data_Receive_Handler(uint8_t* Judge_Data_Buff);
static void Data_Send_Handler(uint8_t* Judge_Data_Buff);
...
//void Judge_Data_Lowlevel_Init(void)
//{
//		Judge_Usart_Init(Judge_Data.Judge_Usart_Rx_Buff[0],Judge_Data.Judge_Usart_Rx_Buff[1],Judge_Data.Judge_Usart_Tx_Buff,
//											Data_Receive_Handler,Data_Send_Handler);
//}
...
uint8_t Is_Judge_Online()
{
		return xTaskGetTickCount()-Judge_Data.Judge_Usart_Rx_Timestamp < JUDGE_OFFLINE_TIME_LIMIT;
}
...
void USART6_IRQHandler(void)
{
		...
		if (USART_GetITStatus(USART6, USART_IT_IDLE) != RESET)
    {
			...
			if(DMA_GetCurrentMemoryTarget(DMA2_Stream1) == 0)
			{
					...
					DMA_Cmd(DMA2_Stream1, ENABLE);
					Data_Receive_Handler(Judge_Data.Judge_Usart_Rx_Buff[0]);
			}
			else
			{
					...
					DMA_ClearFlag(DMA2_Stream1, DMA_FLAG_TCIF1 | DMA_FLAG_HTIF1);
					DMA_Cmd(DMA2_Stream1, ENABLE);
					Data_Receive_Handler(Judge_Data.Judge_Usart_Rx_Buff[1]);
...
void DMA2_Stream7_IRQHandler(void)
{
		if(DMA_GetFlagStatus(DMA2_Stream7,DMA_FLAG_TCIF7)!=RESET)
		{
				...
				DMA_Cmd(DMA2_Stream7,DISABLE);
				Data_Send_Handler(Judge_Data.Judge_Usart_Tx_Buff);
		}
}
