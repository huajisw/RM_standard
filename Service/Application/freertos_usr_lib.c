...
BaseType_t ListItem_Init(ListItem_t* ListItem)
{
		vListInitialiseItem(ListItem);
		ListItem->xItemValue = (TickType_t)xSemaphoreCreateBinary();
		if(ListItem->xItemValue)
				return pdFALSE;
		xSemaphoreGive(ListItem->xItemValue);
		return pdTRUE;
}

void ListItem_Lock(ListItem_t* ListItem)
{
		if(ListItem->xItemValue)
			xSemaphoreTake((SemaphoreHandle_t)ListItem->xItemValue,portMAX_DELAY);
}

void ListItem_Unlock(ListItem_t* ListItem)
{
		if(ListItem->xItemValue)
			xSemaphoreGive((SemaphoreHandle_t)ListItem->xItemValue);
}
...
uint8_t Get_Int_Digits_Count(int32_t Val)
{
