...
static void oled_showchar_choicec(uint8_t row, uint8_t col, uint8_t chr)
{
    uint8_t x = col * 6;
    uint8_t y = row * 12;
    uint8_t temp, t, t1;
    ...
    for (t = 0; t < 12; t++)
    {
        ...
        for (t1 = 0; t1 < 8; t1++)
        {
            if (temp&0x80)
                ...
                oled_drawpoint(x, y, Pen_Inversion);
...
static void oled_showstring_choice(uint8_t row, uint8_t col, uint8_t *chr)
{
    uint8_t n =0;

    while (chr[n] != '\0')
    {
        oled_showchar_choicec(row, col, chr[n]);
        col++;
...
void oled_printf_choice(uint8_t row, uint8_t col, char *fmt,...)
{
    uint8_t LCD_BUF[128] = {0};
    uint8_t remain_size = 0;
    va_list ap;
    ...
    oled_showstring_choice(row, col, LCD_BUF);
}
...
void oled_number(uint8_t row, uint8_t col, float *a, uint8_t mode, uint8_t len)
{
			if(*a<0)
			{
				oled_printf(row,col,"-");	
...
{
	int point = 0;
	float Nummm,Angle_Showww;
	Angle_Showww = *a;
	if(Angle_Showww < 0)
	{
		oled_printf(row,col,"-");	
		col = col+1;	
		Angle_Showww = -Angle_Showww;
	}

	
	integer = 	(int)Angle_Showww;
	Nummm = Angle_Showww- integer;
	for(int k = 0;k < decimal_num;k++)
	Nummm = 10 * Nummm;
	decimal = Nummm;
